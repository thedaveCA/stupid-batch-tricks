...
:: Roughly equivalent to the `cdd` command in Take Command.
::
:: Usage: cdd [path]
:: If no path is specified, display the current directory.
...
if "%~1" == "--help" (
    ...
    echo.
    echo Consider using DOSKEY to map CD to this command: 
    echo   DOSKEY CD=CDD $*
    echo.
) else if "%~0" == "~" (
